# Acceptance And Test Plan - Cobold Briefing
...
| --- | --- | --- |
| AC1 | API returns a specific `reason` for each signal. | Backend unit test, Bruno smoke, DPS-like testautomation. |
| AC2 | API risk scoring includes target environment, complexity, and team experience. | Backend unit test, OpenAPI, sample payloads. |
| AC3 | BFF maps backend response fields into the UI-facing DTO. | BFF build, Bruno smoke, DPS-like testautomation. |
| AC4 | UI renders signal, reason, next action, evidence prompts, checklist, and runtime status. | Browser evidence and UI automation. |
| AC5 | Representative `truce`, `sparring`, and `shield-wall` cases are covered. | Bruno smoke and DPS-like testautomation. |
| AC6 | HLD/LLD explain behavior before implementation. | Solution review against `solution/cobold-briefing/`. |
| AC7 | Scope stays inside briefing behavior and verification surfaces. | Diff boundary review. |
...
| Browser evidence | Playwright, Agent Browser, or manual screen share | See browser checks below | Tester / reviewer |
...
- `01-truce-briefing` returns `200`, `signal=truce`, and a clear verification
  reason.
- `02-sparring-briefing` returns `200`, `signal=sparring`, and evidence prompts.
- `03-shield-wall-briefing` returns `200`, `signal=shield-wall`, and a
...
- Representative pytest cases pass.
...
- Bruno smoke result
- DPS-like testautomation result
